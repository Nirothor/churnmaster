...
# Define a new pipeline step to extract insights from customer feedback
def extract_insights(text_data):
    insights = []
    for text in text_data:
        response = meta_ai.classify_text(
            text=text,
            classification_type="churn-risk"
        )
        insights.append(response.json())
    return insights

# Add the new pipeline step
pipeline.steps.append(('extract_insights', extract_insights))
